...
async function main() {
  ...
  const extraAuthorInput = await ask(
    "Additional author slugs (comma-separated, or blank)",
    "",
  );
  const authorSlugs = [
    authorSlug,
  ...
  const sortTitle = await ask(
    'Sort title (omit leading "The", "A", etc.)',
    title,
  );
  const bookSlug = toSlug(title);
  ...
  const formats = await askMultiChoice(
    "Which formats exist?",
    availableFormats,
  );
  ...
  if (formats.includes("Print-Ready ZIP"))
    console.log(`  content/print-ready.zip`);
  if (formats.includes("Print/Amazon"))
